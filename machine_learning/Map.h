...
public:
  ...
  //       using "Value_type()".
    Iterator find(const Key_type& k) const {
        Pair_type input{k, Value_type()};
        return variable.find(input);
    }
  ...
  // HINT: http://www.cplusplus.com/reference/map/map/operator[]/
    Value_type& operator[](const Key_type& k) {
        Pair_type input{k, Value_type()};
        std::pair<Iterator, bool> result = insert(input);
  ...
  //           the value true.
    std::pair<Iterator, bool> insert(const Pair_type &val) {
        Iterator place = find(val.first);
        if (place != end()) {
            return {place, false};
        }
        else {
            variable.insert(val);
            Iterator new_place = find(val.first);
